...
export class Auth extends Component {
  ...
  myChangeHandler = (event) => {
    ...
    switch (name) {
      case 'email':
        if (value.length === 0) {
          errors.email = value.length < 5 ? 'Заавал мейлээ бичнэ үү!' : ''
          break
        }
        if (value.length > 0) {
          errors.email = validEmailRegex.test(value) ? '' : 'Мейл буруу байна!'
          break
        }
        break
      case 'password':
        if (value.length > 0) {
          errors.password =
            value.length < 6 ? 'Нууц үг 6-с дээш үсэг, тоо агуулсан байна!' : ''
        }

        if (value.length === 0) {
          errors.password =
            value.length === 0 ? 'Нууц үгээ заавал бичнэ үү!' : ''
        }
        break
      default:
        break
      ...
      iserror = (
        <>
          <div className='container error container-short'>
            <div className='mar-20'>
              <h5>Алдаа гарлаа - {this.state.error.code}</h5>
      ...
      <>
        ...
        <div className='container container-short py-5'>
          <h1 className='pt-2 py-2'>
            {this.state.isLoginMode ? 'Нэвтрэх хэсэг ' : 'Бүртгүүлэх хэсэг'}
          </h1>
          <hr></hr>
          <form onSubmit={this.mySubmitHandler} className='pt-4'>
            <div className='form-group'>
              <label htmlFor='email'>Мейл </label>
              <input
                type='email'
                name='email'
            ...
            <div className='form-group'>
              ...
              >
                {this.state.isLoginMode ? 'Нэвтрэх' : 'Бүртгүүлэх'}
              </button>
              ...
              >
                {this.state.isLoginMode ? 'Бүртгүүлэх' : 'Нэвтрэх'} хэсэг рүү
                очих
              </button>
            </div>
          </form>
